...
public class LongestConsecutiveSequence {
     ...
     * 那么next-pre-1就是当前数字的最长连续序列，更新res即可。这里再说下，为啥当检测某数字在集合中存在当时候，都要移除数字。这是为了避免大量的重复计算，
     * 就拿题目中的例子来说吧，我们在遍历到4的时候，会向下遍历3，2，1，如果都不移除数字的话，遍历到1的时候，还会遍历2，3，4。同样，
     * 遍历到3的时候，向上遍历4，向下遍历2，1，等等等。如果数组中有大量的连续数字的话，那么就有大量的重复计算，十分的不高效，所以我们要从HashSet中移除数字
     */
    //HashSet
    ...
    public int longestConsecutive02(int[] nums){
        int result = 0;
        HashMap<Integer,Integer> map = new HashMap<>();  //map的key是num的值，value是遍历时的最长连续的数值
        for(int num : nums){
            if (map.containsKey(num))  //如果map已存在该key，直接跳过，避免重复计算
                continue;
