...
## 10.内嵌标签
iframe  
src:要显示的网页资源路径,可以是本地(相对路径)也可以是网络资源(URL).**注意*:默认当前页面打开及加载src指向的资源.  
width:设置显示区域的宽度.  
height:设置显示区域的高度.  
...
**注意**：
- 表单数据的提交,要提交的表单项**必须拥有name**属性值,否则不会提交；提交的表单项数据为键值对,键为name属性的值,值为用户书写的数据。
- form标签会收集其标签内部的数据
- form表单的数据提交需要依赖于**submit提交按钮**.
### （2）form表单域标签
...
select
- name:数据提交的键名,必须声明
- 子标签option:一个option标签表示一个下拉选项；value:要提交的数据
#### 5）文本域
